...
static UDPContext *gUdpContext2;
...
void rtpSendData(RTPMuxContext *ctx, const uint8_t *buf, int len, int mark)
{
    ...
    // send for parse rtp to nal 
    res = udpSend(gUdpContext2,ctx->cache, (uint32_t)(len + 12));
...
static void rtpSendNAL(RTPMuxContext *ctx, const uint8_t *nal, int size, int last){
    ...
    //printf("---rtpSendNAL  len = %d M=%d，aggregation=%d\n", size, last,ctx->aggregation);

    // Single NAL Packet or Aggregation Packets
    if (size <= RTP_PAYLOAD_MAX){

        printf("---rtpSendNAL  less then  RTP_PAYLOAD_MAX\r\n");
        // Aggregation Packets
        if (ctx->aggregation){

            //printf("---rtpSendNAL  less then  RTP_PAYLOAD_MAX aggregation :%d\r\n",ctx->aggregation);
            ...
            printf("WWWWWWWWWWWWWhy be here   out:%d!\r\n",(int)(ctx->buf_ptr - ctx->buf));

            // meet last NAL, send all buf
            if (last == 1){
                printf("WWWWWWWWWWWWWhy be here !\r\n");
        ...
        // Single NAL Unit RTP Packet
             ...
             *  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
             * */
             printf("DDDDDDDDDDDDDDDD be here !\r\n");

            rtpSendData(ctx, nal, size, last);
        }

    } else {  // 分片分组
        printf("---rtpSendNAL  more then  RTP_PAYLOAD_MAX!!!!Split!!!\r\n");
        /*
         *
         *  0                   1                   2
        ...
        if (ctx->buf_ptr > ctx->buf){
            ...
            printf("DDDDDDDDDDDDDDDD be here real send data_len:%d !\r\n",(int)(ctx->buf_ptr - ctx->buf));
        ...
        rtpSendData(ctx, buff, size + headerSize, 1);
    }
}

// 从一段H264流中，查询完整的NAL发送，直到发送完此流中的所有NAL
void rtpSendH264HEVC(RTPMuxContext *ctx, UDPContext *udp, UDPContext *udp2, const uint8_t *buf, int size){
    const uint8_t *r;
